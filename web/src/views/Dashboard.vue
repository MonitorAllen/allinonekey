...
<template>
  <div class="min-h-screen bg-gray-950 text-gray-100 flex flex-col md:flex-row font-sans">
    ...
    <aside class="w-full md:w-64 bg-gray-900 border-r border-gray-800 flex md:flex-col md:fixed md:h-full z-20">
      <div class="p-6 text-blue-400 font-bold text-xl flex items-center gap-2"><ShieldCheck class="w-6 h-6" /> AllInOne</div>
      <nav class="flex-1 px-4 py-2 md:py-0 flex md:block gap-1 overflow-x-auto md:space-y-1">
        <button @click="activeTab = 'keys'" :class="['w-full flex items-center gap-3 px-4 py-3 rounded-lg', activeTab === 'keys' ? 'bg-blue-600/20 text-blue-400' : 'text-gray-400']"><Key class="w-5 h-5" /> AI API Keys</button>
        <button @click="activeTab = 'accounts'" :class="['w-full flex items-center gap-3 px-4 py-3 rounded-lg', activeTab === 'accounts' ? 'bg-blue-600/20 text-blue-400' : 'text-gray-400']"><Lock class="w-5 h-5" /> Accounts Vault</button>
        <button @click="activeTab = 'logs'" :class="['w-full flex items-center gap-3 px-4 py-3 rounded-lg', activeTab === 'logs' ? 'bg-blue-600/20 text-blue-400' : 'text-gray-400']"><Activity class="w-5 h-5" /> Audit Logs</button>
        <div v-if="auth.role === 'admin'" class="pt-4 mt-4 border-t border-gray-800"><p class="px-4 text-[10px] text-gray-500 uppercase mb-2">Admin Tools</p><button @click="activeTab = 'admin'" :class="['w-full flex items-center gap-3 px-4 py-3 rounded-lg', activeTab === 'admin' ? 'bg-blue-600/20 text-blue-400' : 'text-gray-400']"><Ticket class="w-5 h-5" /> Invitations</button></div>
      </nav>
      <div class="p-4 border-t border-gray-800"><button @click="handleLogout" class="w-full flex items-center gap-3 px-4 py-3 text-gray-400 hover:text-red-400"><LogOut class="w-5 h-5" /> Logout</button></div>
    </aside>

    <main class="flex-1 md:ml-64 p-4 md:p-8 space-y-6">
      <div class="flex flex-col xl:flex-row gap-4 xl:justify-between xl:items-center">
        <div class="relative max-w-md w-full"><Search class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" /><input v-model="searchQuery" placeholder="Search..." class="w-full bg-gray-900 border-gray-800 rounded-xl py-3 pl-10 pr-4 text-sm outline-none border focus:border-blue-500" /></div>
        <div class="flex flex-wrap gap-3">
          <button @click="exportData(activeExportScope, 'json')" class="px-4 py-3 bg-gray-900 border border-gray-800 rounded-xl hover:text-blue-400 text-sm flex items-center gap-2"><FileDown class="w-4 h-4"/>Export JSON</button>
          <button @click="exportData(activeExportScope, 'csv')" class="px-4 py-3 bg-gray-900 border border-gray-800 rounded-xl hover:text-blue-400 text-sm flex items-center gap-2"><FileDown class="w-4 h-4"/>Export CSV</button>
          ...
          <button v-if="importFile && (isKeysTab || isAccountsTab)" @click="importData(activeExportScope)" class="px-4 py-3 bg-green-600 rounded-xl hover:bg-green-700 text-sm">Run Import</button>
          ...
          <button v-if="isKeysTab" @click="openNewKeyModal" class="bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded-xl font-bold flex items-center gap-2 text-sm"><Plus class="w-5 h-5"/> Add Keys</button>
          <button v-if="isAccountsTab" @click="openNewAccountModal" class="bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded-xl font-bold flex items-center gap-2 text-sm"><Plus class="w-5 h-5"/> New Account</button>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'accounts'" class="space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-5">
          <div v-for="a in accounts" :key="a.id" class="relative overflow-hidden bg-gradient-to-br from-gray-900 to-gray-950 p-5 rounded-2xl border border-gray-800 hover:border-blue-500/40 shadow-xl shadow-black/20 transition-all">
            <div class="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-blue-500/70 via-green-400/50 to-transparent"></div>
      ...
      <div v-if="activeTab === 'logs'" class="space-y-4">
        <div class="bg-gray-900 rounded-2xl border border-gray-800 p-4 grid grid-cols-1 md:grid-cols-3 gap-3"><select v-model="auditFilters.action" class="bg-gray-950 border border-gray-800 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-500"><option value="">All actions</option><option value="ADD_KEY">ADD_KEY</option><option value="UPDATE_KEY">UPDATE_KEY</option><option value="DELETE_KEY">DELETE_KEY</option><option value="CREATE_ACCOUNT">CREATE_ACCOUNT</option><option value="UPDATE_ACCOUNT">UPDATE_ACCOUNT</option><option value="EXPORT_KEYS_JSON">EXPORT_KEYS_JSON</option><option value="IMPORT_KEYS_CSV">IMPORT_KEYS_CSV</option></select><input v-model="auditFilters.keyword" placeholder="Search detail / IP..." class="md:col-span-2 bg-gray-950 border border-gray-800 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-500" /></div>
        <div class="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden"><table class="w-full text-left"><thead class="bg-gray-800/50 text-xs uppercase text-gray-500"><tr><th class="px-6 py-4">Action</th><th class="px-6 py-4">IP</th><th class="px-6 py-4">Time</th></tr></thead><tbody class="divide-y divide-gray-800"><tr v-if="loadingLogs"><td colspan="3" class="px-6 py-10 text-center text-gray-500">Loading logs...</td></tr><tr v-for="log in auditLogs" :key="log.id" class="hover:bg-gray-800/20"><td class="px-6 py-4"><span class="text-sm font-bold text-blue-400">{{ log.action }}</span><p class="text-xs text-gray-500">{{ log.detail }}</p></td><td class="px-6 py-4 text-xs text-gray-400">{{ log.ip }}</td><td class="px-6 py-4 text-xs text-gray-500">{{ new Date(log.created_at).toLocaleString() }}</td></tr><tr v-if="!loadingLogs && auditLogs.length === 0"><td colspan="3" class="px-6 py-10 text-center text-gray-600">No audit logs found.</td></tr></tbody></table><div class="flex items-center justify-between border-t border-gray-800 px-6 py-4 text-sm text-gray-400"><span>Total {{ auditTotal }} logs · Page {{ auditPage }} / {{ auditTotalPages }}</span><div class="flex gap-2"><button @click="changeAuditPage(auditPage - 1)" :disabled="auditPage <= 1 || loadingLogs" class="px-3 py-1 rounded-lg border border-gray-800 disabled:opacity-40 hover:text-blue-400">Prev</button><button @click="changeAuditPage(auditPage + 1)" :disabled="auditPage >= auditTotalPages || loadingLogs" class="px-3 py-1 rounded-lg border border-gray-800 disabled:opacity-40 hover:text-blue-400">Next</button></div></div></div>
      </div>
      ...
      <div v-if="activeTab === 'admin'" class="space-y-6">
        ...
        <div class="bg-gray-900 rounded-2xl border border-gray-800 p-4"><select v-model="inviteFilters.status" class="bg-gray-950 border border-gray-800 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-500"><option value="">All invites</option><option value="available">Available</option><option value="used">Used</option><option value="expired">Expired</option></select></div>
        <div class="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden"><table class="w-full text-left"><thead class="bg-gray-800/50 text-xs uppercase text-gray-500"><tr><th class="px-6 py-4">Code</th><th class="px-6 py-4">Status</th><th class="px-6 py-4">Expires</th><th class="px-6 py-4">Used By</th><th class="px-6 py-4 text-right">Actions</th></tr></thead><tbody class="divide-y divide-gray-800"><tr v-if="loadingInvites"><td colspan="5" class="px-6 py-10 text-center text-gray-500">Loading invites...</td></tr><tr v-for="i in invites" :key="i.id"><td class="px-6 py-4 font-mono text-sm text-blue-300">{{ i.code }}</td><td class="px-6 py-4"><span :class="['px-2 py-0.5 rounded-full text-[10px] font-bold uppercase', i.is_used ? 'text-red-400 bg-red-400/10' : 'text-green-400 bg-green-400/10']">{{ i.is_used ? 'Used' : 'Available' }}</span></td><td class="px-6 py-4 text-xs text-gray-500">{{ i.expires_at ? new Date(i.expires_at).toLocaleString() : '-' }}</td><td class="px-6 py-4 text-xs text-gray-500">{{ i.used_by || '-' }}</td><td class="px-6 py-4 text-right"><button v-if="!i.is_used" @click="deleteInvite(i.id)" class="text-gray-600 hover:text-red-400"><Trash2 class="w-4 h-4"/></button></td></tr><tr v-if="!loadingInvites && invites.length === 0"><td colspan="5" class="px-6 py-10 text-center text-gray-600">No invites found.</td></tr></tbody></table><div class="flex items-center justify-between border-t border-gray-800 px-6 py-4 text-sm text-gray-400"><span>Total {{ inviteTotal }} invites · Page {{ invitePage }} / {{ inviteTotalPages }}</span><div class="flex gap-2"><button @click="changeInvitePage(invitePage - 1)" :disabled="invitePage <= 1 || loadingInvites" class="px-3 py-1 rounded-lg border border-gray-800 disabled:opacity-40 hover:text-blue-400">Prev</button><button @click="changeInvitePage(invitePage + 1)" :disabled="invitePage >= inviteTotalPages || loadingInvites" class="px-3 py-1 rounded-lg border border-gray-800 disabled:opacity-40 hover:text-blue-400">Next</button></div></div></div>
      </div>
    </main>

    <div v-if="showKeyModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div class="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-800 flex justify-between items-center"><h3 class="text-xl font-bold">{{ keyModalTitle }}</h3><button @click="showKeyModal = false"><X class="w-6 h-6"/></button></div>
        <div class="p-6 space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label class="text-xs font-bold text-gray-500 uppercase">Provider</label><select v-model="keyForm.provider" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm"><option>OpenAI</option><option>DeepSeek</option><option>Anthropic</option><option>Gemini</option><option>Custom</option></select></div><div><label class="text-xs font-bold text-gray-500 uppercase">Pool Group</label><input v-model="keyForm.pool_group" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm" /></div></div>
          <div v-if="keyForm.provider === 'Custom'"><label class="text-xs font-bold text-gray-500 uppercase">Custom Provider Name</label><input v-model="keyForm.custom_provider" placeholder="one-api / new-api / company-relay" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm" /></div>
          <div v-for="(row, idx) in keyForm.keys" :key="idx" class="grid grid-cols-1 md:grid-cols-[1fr_1.4fr_auto] gap-3 bg-gray-950/50 border border-gray-800 rounded-xl p-3"><input v-model="row.key_name" placeholder="Custom key name" class="bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm" /><input v-model="row.key_value" :placeholder="editingKeyId ? 'Leave empty to keep current key value' : 'API key value'" class="bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm font-mono" /><button v-if="!editingKeyId" @click="removeKeyRow(idx)" class="text-gray-500 hover:text-red-400"><Trash2 class="w-4 h-4"/></button></div>
          <button v-if="!editingKeyId" @click="addKeyRow" class="px-3 py-2 border border-dashed border-gray-700 rounded-lg text-sm text-gray-400 hover:text-blue-400 hover:border-blue-500">+ Add another row</button>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label class="text-xs font-bold text-gray-500 uppercase">Base URL</label><input v-model="keyForm.base_url" :placeholder="keyForm.provider === 'Custom' ? 'https://relay.example.com' : 'Optional provider override'" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm" /></div><div><label class="text-xs font-bold text-gray-500 uppercase">Proxy URL</label><input v-model="keyForm.proxy_url" placeholder="Optional: http://127.0.0.1:7890" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm" /></div></div>
          <div v-if="editingKeyId"><label class="text-xs font-bold text-gray-500 uppercase">Status</label><input v-model="keyForm.status" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm" /></div>
        </div>
        <div class="p-6 bg-gray-800/30 flex justify-end gap-3"><button @click="saveKeys" class="bg-blue-600 px-6 py-2 rounded-lg font-bold">Save</button></div>
      </div>
    </div>
    ...
    <div v-if="showAccountModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div class="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-md shadow-2xl">
        <div class="p-6 border-b border-gray-800 flex justify-between items-center"><h3 class="text-xl font-bold">{{ accountModalTitle }}</h3><button @click="showAccountModal = false"><X class="w-6 h-6"/></button></div>
        <div class="p-6 space-y-4"><div><label class="text-xs font-bold text-gray-500 uppercase">Platform</label><input v-model="accountForm.platform" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm" /></div><div><label class="text-xs font-bold text-gray-500 uppercase">Account</label><input v-model="accountForm.account" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm" /></div><div><label class="text-xs font-bold text-gray-500 uppercase">Password</label><input v-model="accountForm.password" type="password" :placeholder="editingAccountId ? 'Leave empty to keep current password' : ''" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm" /></div><div><label class="text-xs font-bold text-gray-500 uppercase">URL</label><input v-model="accountForm.url" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm" /></div><div><label class="text-xs font-bold text-gray-500 uppercase">TOTP Secret</label><input v-model="accountForm.totp_secret" placeholder="Optional raw base32 secret" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm" /></div><div><label class="text-xs font-bold text-gray-500 uppercase">Favicon URL</label><input v-model="accountForm.favicon_url" placeholder="Optional override" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm" /></div></div>
        <div class="p-6 bg-gray-800/30 flex justify-end gap-3"><button @click="saveAccount" class="bg-blue-600 px-6 py-2 rounded-lg font-bold">Save Account</button></div>
      </div>
    </div>
  </div>
</template>
