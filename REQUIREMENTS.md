...
- [x] 按需解密并复制到剪贴板。
- [x] 按用户隔离查询。
- [x] 基础搜索：按 provider / key_name 查询。
- [x] 状态统计：total / active / error / balance。
- [x] API Keys 模块独立刷新，不触发 Accounts 刷新。
- [x] OpenAI / OpenAI-compatible Key 健康探测：调用 `/v1/models` 验证密钥与 Base URL 可用性，状态写入 `active` / `auth_error` / `rate_limited` / `quota_error`。
- [x] DeepSeek Key 健康探测：调用 `/v1/models` 验证密钥与 Base URL 可用性，状态写入 `active` / `auth_error` / `rate_limited` / `quota_error`。
- [x] Anthropic / Claude Key 健康探测：调用 `/v1/models` 验证密钥与 Base URL 可用性。
- [x] Gemini Key 健康探测：调用 `/v1beta/models` 验证密钥与 Base URL 可用性。
- [x] 自定义 Provider / 第三方中转站适配：支持前端填写自定义 provider 名称与 Base URL；后端在未知 provider 但存在 Base URL 时按 OpenAI-compatible `/v1/models` 探测。
- [x] 代理 URL 局部生效逻辑：API Key 支持 `proxy_url`，健康探测时仅当前 Key 生效。
- [x] Key 编辑能力完善，支持 provider / pool_group / key_name / base_url / status / key_value 更新。
...
| POST | `/api/keys/bulk` | 批量导入 API Key，支持 `base_url` 与 `proxy_url`。 |
...
| GET | `/api/export/json` | 导出当前用户加密 JSON 数据。 |
| GET | `/api/export/csv` | 导出当前用户加密 CSV 数据。 |
| POST | `/api/import/json` | 导入加密 JSON 数据并归属到当前用户。 |
