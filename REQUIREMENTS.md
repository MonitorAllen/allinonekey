...
系统用于统一管理多服务商 AI API Key、按池子分组、批量导入、Key 健康探测、个人账号密码保险箱、邀请码注册、多用户隔离与审计日志。真实额度/用量查询暂缓，后续仅在接入官方 usage/cost/billing API 或 new-api 这类明确支持额度查询的中转系统后再推进。
...
- **版本规范优先**：项目采用 SemVer 风格的 `MAJOR.MINOR.PATCH` 三段式版本号，首个公开体验版为 `0.1.0`。
...
- 默认服务端口：`8080`
- 本地开发前端端口：`5173`
- 环境变量支持通过进程环境或本地 `.env` 文件加载；仓库只托管 `.env.example`，不托管真实 `.env`。
...
### 5.5 AccountCredential

- `id`
- `user_id`
- `account_id`
- `name`
- `value`：AES-256-GCM 密文
- `note`
- `expires_at`
- `created_at`
- `updated_at`
...
- [x] Key 编辑能力完善，支持 provider / pool_group / key_name / base_url / proxy_url / provider_url / note / status / key_value 更新。
- [x] Key 健康详情展示完善：列表展示 status、last_check、base_url、proxy_url，并在健康探测后刷新；`quota_total` / `quota_used` / `quota_balance` 仅在接入服务商官方用量/费用 API 后写入真实值，当前不得用 `/v1/models` 结果伪造额度。
- [x] 每个 API Key 支持 Provider 官网地址、官网跳转与根据官网地址自动生成 favicon；provider favicon 展示在提供商分组名称左侧，官网跳转仅以提供商名称后的按钮呈现；每个 API Key 支持备注字段，备注只保留在 Key 行内维护，新增/编辑单个 Key 可维护独立备注，Add Provider 多行新增时每行也可单独填写备注；新增 Provider 时可一次添加多个 Key，Add Provider 面板标题明确为 Add Provider，并保持可维护 provider_url / base_url / proxy_url 等 Provider 元数据；Add Provider / Add Key 面板使用加宽布局，Key Value 输入框优先展示完整提示与长 Key 值。
- [x] 向已有 Provider 添加 Key 时只填写 Key Name、Key Value、Pool Group 与 Key 备注，并只读展示当前 Provider Name 作为上下文提示；不再重复展示、提供或修改 Custom Provider Name、Provider 官网、base_url、proxy_url 等平台级信息；这些 Provider 元数据统一通过 Provider 编辑入口维护，与 Accounts 的 Add Account / Edit Platform 逻辑保持一致。
- [x] 每个 API Key 支持按需查询 Provider 模型列表；OpenAI-compatible / DeepSeek / Anthropic 使用 `/v1/models`，Gemini 使用 `/v1beta/models`，Custom Provider 需要配置 `base_url` 后按 OpenAI-compatible 模式查询。模型列表只代表该 Key 可访问的模型集合，不代表真实额度或用量。
- [x] API Keys 支持按模块导出 JSON/CSV，并支持 JSON/CSV 导入。

### 6.2 个人账号密码管理

- [x] 平台、账号、密码、URL 存储。
- [x] Accounts 采用 AccountPlatform / AccountItem 两层模型：同一平台可添加多个账号，UI 按平台分组并在平台面板内新增账号，新增层次与 AI API Keys 的“新增提供商 / 向已有提供商添加 Key”保持一致。
- [x] Accounts 新增平台时支持同时添加第一个账号，避免平台与账号分两步创建；Add Account 只填写账号、密码、TOTP 与备注，不再重复填写平台 URL / Icon，这些平台元数据统一由创建或编辑平台时维护。
- [x] Accounts 支持修改平台名称、官网 URL、favicon 与备注。
- [x] Accounts 支持删除平台与账号：删除平台时级联删除平台下账号与账号凭据；删除账号时先删除账号下凭据再删除账号本体；旧版扁平 `accounts` 表数据在懒迁移后会删除已迁移旧行，避免删除后又被旧数据重新迁移“复活”。
- [x] AccountItem 支持账号级备注 `note`。
- [x] AccountItem 支持多个账号级 auth token / API key 凭据；凭据不单独增加类型字段，由 Owner 通过名称或备注标记；凭据支持备注与 `expires_at`，过期需要在 UI 中提示。
- [x] 密码、TOTP Secret、账号级 auth token / API key 均使用 AES-256-GCM 加密存储。
- [x] 前端卡片式展示。
- [x] 密码按需解密并复制。
- [x] 按用户隔离查询。
- [x] Accounts 模块独立刷新，不触发 API Keys 刷新。
- [x] Accounts 列表样式优化：深色卡片、favicon、URL、密码复制、TOTP 与操作按钮分区展示；平台下账号列表默认展示前三个，超过三个时支持展开 / 收起。
...
- [x] 网站图标自动抓取 Favicon，支持手动覆盖 favicon_url；官网跳转图标展示在链接文本后面。
...
| GET | `/api/system/info` | 获取系统公开元信息，目前返回版本号。 |
...
| POST | `/api/keys/create` | 按需新增 Provider 或向既有 Provider 添加一个或多个 API Key；每个条目必须提供 `key_name` 与 `key_value`，每个 Key 可单独携带 `note`；新增 Provider 时支持 `base_url`、`proxy_url`、`provider_url`，并根据 `provider_url` 自动生成 `provider_icon`。 |
| POST | `/api/keys/bulk` | 兼容旧批量导入路径；新前端主入口使用 `/api/keys/create` 的多行表单。 |
| POST | `/api/keys/:id/check-quota` | 手动触发单个 API Key 健康探测；OpenAI-compatible / DeepSeek 使用 `/v1/models`，Anthropic 使用 `/v1/models`，Gemini 使用 `/v1beta/models`。未知 provider 只要配置了 `base_url`，按 OpenAI-compatible 中转站探测 `/v1/models`。当前接口只证明 Key 可认证与 Provider 可达，不代表真实额度；`quota_*` 字段只有后续接入官方 usage/cost/billing API 时才写入真实数值。 |
| GET | `/api/keys/:id/models` | 按需查询单个 API Key 可访问的模型列表；OpenAI-compatible / DeepSeek / Anthropic 使用 `/v1/models`，Gemini 使用 `/v1beta/models`，Custom Provider 需要配置 `base_url` 后按 OpenAI-compatible 模式查询。该接口返回 `status`、`models` 与可选 `error`，不返回或推断额度。 |
| PATCH | `/api/keys/:id` | 更新 API Key 元数据、`base_url`、`proxy_url`、`provider_url`、`note` 或重新加密 key_value。 |
...
| Method | Path | 说明 |
|---|---|---|
| GET | `/api/accounts/list` | 获取当前用户账号平台列表，返回 AccountPlatform 及其 AccountItems 与 AccountCredentials，支持 `q` 搜索 platform / account / url / note / credential name。 |
| POST | `/api/account-platforms/create` | 新增账号平台。 |
| PATCH | `/api/account-platforms/:id` | 更新账号平台元数据。 |
| DELETE | `/api/account-platforms/:id` | 删除账号平台及其下账号与凭据。 |
| POST | `/api/accounts/create` | 在指定平台下新增账号密码。 |
| PATCH | `/api/accounts/:id` | 更新账号，密码与 TOTP Secret 字段会重新加密。 |
| DELETE | `/api/accounts/:id` | 删除账号及其下凭据。 |
| GET | `/api/accounts/:id/decrypt` | 按需解密指定账号密码。 |
| GET | `/api/accounts/:id/totp` | 按需生成账号 TOTP 动态验证码。 |
| POST | `/api/accounts/:id/credentials` | 为指定账号新增 auth token / API key 凭据，value 加密存储，支持 `note` 与 `expires_at`。 |
| PATCH | `/api/account-credentials/:id` | 更新账号级凭据，value 字段存在时重新加密。 |
| DELETE | `/api/account-credentials/:id` | 删除账号级凭据。 |
| GET | `/api/account-credentials/:id/decrypt` | 按需解密并复制账号级凭据。 |
...
- `listKeyModels()`：只在用户点击单个 Key 的 Models 操作时拉取该 Key 可访问模型列表，不触发 Accounts / Audit / Admin 模块刷新；同一个 Key 的模型列表在前端内存缓存，重复展开直接复用缓存结果；模型列表按模型名称首字母排序，支持按响应结果中的模型名、ID、owned_by 与 Provider 进行前端搜索；模型列表以内联下拉区域展示，不使用弹窗；模型卡片主标题直接展示模型名，下方保留供应商信息，点击模型卡片可复制模型名称并显示复制成功反馈。
- `fetchAccounts()`：只拉取账号平台列表、平台下账号列表与账号级凭据；Accounts 搜索框只触发 Accounts 列表请求，不触发 Keys / Stats。
...
后端运行时版本优先读取 `ALLINONEKEY_APP_VERSION`，前端构建期版本可读取 `VITE_ALLINONEKEY_APP_VERSION`，默认均为 `0.2.0`。
...
- 前端 `web/package.json` 中的 `version` 与项目版本保持一致，首个公开体验版为 `0.1.0`。
...
- 真实额度/用量查询不阻塞首个公开体验版本；本阶段仅保留 Key 健康探测。
...
ALLINONEKEY_DB_PATH=data/allinone.db ALLINONEKEY_APP_VERSION=0.2.0 ALLINONEKEY_JWT_SECRET=*** ALLINONEKEY_SESSION_SECRET=*** /usr/local/go/bin/go run ./cmd/server/main.go
```

本地默认数据库路径为 `data/allinone.db`。`make dev` 与 `make dev-server` 会自动创建 `data/` 目录，并通过 `ALLINONEKEY_DB_PATH` 指向该文件。后端启动时会尝试读取项目根目录 `.env`，但真实 `.env` 不托管；仓库只保留 `.env.example`。
...
cd /home/allen/WorkSpace/Go/src/allinonekey
cp .env.example .env
# 修改 .env 中的 ALLINONEKEY_JWT_SECRET / ALLINONEKEY_SESSION_SECRET 为强随机值
docker-compose up --build -d
# 或：
ALLINONEKEY_JWT_SECRET=*** ALLINONEKEY_SESSION_SECRET=*** make docker-up
```
...
- [x] 真实额度/用量查询暂缓，不阻塞首个公开体验版；Key 健康探测继续保留。
...
- [x] 新增 `.env.example` 作为环境变量示例文件；真实 `.env` 被 `.gitignore` 排除，Docker Compose 支持从 `.env` 读取变量并显式覆盖容器内 DB 路径、端口、版本与 secret。
...
- [x] 真实额度/用量查询暂缓：官方 Provider 通常不是按普通模型调用 Key 暴露“余额查询”，后续若推进，应单独建模 Provider Billing Credentials / new-api Key 接入；继续保持与 Key 健康探测分离，不影响首个公开体验版本发布。
...
- 单个 API Key 可手动触发健康探测；探测结果写入状态字段。真实额度必须来自服务商官方 usage/cost/billing API；若 Provider 不提供可用接口或权限不足，不得伪造 balance。
- 自定义 Provider / 第三方中转站可以通过自定义名称 + Base URL 导入，并按 OpenAI-compatible `/v1/models` 完成健康探测。
- API Key 可按 Provider / Pool Group 展示。
- AI API Keys 顶部主按钮语义为新增 Provider；已存在 Provider 面板内提供 Add Key 操作，可直接向该 Provider 添加 Key。
- 新增 AccountPlatform 后，Accounts 列表立即展示平台面板；新增 AccountItem 后，该平台面板立即展示账号；新增账号级凭据后，所属账号立即展示凭据。
...
- AccountCredential 可保存多个加密 auth token / API key，支持备注、过期时间与过期提示，并可按需解密复制。
...
- `GET /api/accounts/list` 不允许 404，且 `q` 搜索应能按 platform / account / url / note / credential name 过滤。
